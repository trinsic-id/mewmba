...
import {ConversationsListResponse, UsersListResponse, ChannelsListResponse} from "@slack/web-api";
...
describe("Slack connections", () => {
  ...
  it("Get Users List", async () => {
    ...
    console.log(usersList.members!.map((x: Member) => `{ "githubUsername": "", "slackName":"${x.name}", "slackId":"${x.id}" },`))
  });
  it("Get Channel ID List", async () => {
    const channelsList: ConversationsListResponse = await mySlack.getChannelsList();
    expect(channelsList.channels).not.toBeNull();

    console.log(channelsList.channels!.map(x => `{ "channelId": "${x.id}", "channelName":"${x.name}" },`))
  });
  afterAll(() => {
    mySlack.disconnect()
  });
